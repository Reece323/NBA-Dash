...
from src.br_scraper import get_gamelogs, get_season_stats
from src.figures_player import fig_dict
...
data_selection = html.Div([
    html.H4('Choose Player/Season', className="card-title"),
    dbc.Row([ 
        dcc.Dropdown(
            id='player-id-drop',
            options=id_dict,
...
@app.callback([Output(f'fig-1-data-type', 'options')], #{i} for i in range(1, 4)],
...
@app.callback([Output(f'fig-1-fig-type', 'options')], #{i} for i in range(1, 4)],
              [Input(f'fig-1-data-type', 'value')], #{i}for i in range(1, 4)],
              prevent_initial_call=True)
